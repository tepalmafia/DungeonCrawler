...
const Game = {
  ...
  floorHpScale() {
    ...
    // ★ v213 검토 — 스펀지(방 치우는 시간 5층 25.9초)를 걷으려 ×1.88/×3.58 로 낮춰 봤으나,
    //   봇 런은 특성 뽑기마다 화력이 공1.6~5.2 로 흔들려 표본 18방으로는 **변화와 잡음을
    //   구분할 수 없었다**(고친 뒤가 오히려 24.6초/48.1초). 검증 못 한 밸런스는 올리지 않는다.
    //   위협 쪽(적 피해량, combat.js)만 먼저 올리고, 이 곡선은 사장 실측 빌드로 모델을
    //   고정한 뒤에 다시 본다
...
(async function boot() {
  ...
  function frame(now) {
    ...
    Input.pollTouch();   // v204 — 터치 스틱을 WASD 로 번역 (게임패드와 같은 방식)
    while (acc >= STEP) {

...
const GameCombat = {
  ...
  damageEnemy(e, dmg, dir, { feel = true, crit = false, kb = 190, color } = {}) {
    ...
    // 4층32s 5층25s 6층21s 7층17s **8층44s** 9층24s 10층34s. 철갑 보스(3·8층)만 이웃의 2~3배.
    // 층별 실측(공격력 3배 대조):
    //   3층 190HP 공4 → 피해 2 (3배로 올려도 **2**) = 95타 · 성장 배수 1.00
    //   8층 650HP 공9 → 피해 2 = 325타 · 30층 3520HP 공30 → 피해 2 = **1760타**
    //   45층 3600HP 공45 → 피해 2 = **1800타**  ← 직격 무기가 사실상 무의미
    // 기믹 문구는 '강한 일격을 **경감**한다'인데 실제로는 화력을 통째로 지우고 있었다.
    // v158에서 버스트 상한을 두고 고친 그 실수(성장 무효화)가, 더 오래된 이 축에 그대로 남아 있었다.
    // 잡몹은 HP가 작아(6~14) 클램프가 곧 '몇 대'로 끝나므로 종전 그대로 둔다.
    if (e.armorCap && feel) {
      if (e.isBoss) {
        const mit = e.armorCap >= 3 ? 0.55 : 0.65; // 킷 등급을 경감률로 번역 (직격 -45% / -35%)
        const cut = Math.max(1, Math.round(dmg * mit));
        if (cut < dmg) {
          dmg = cut;
          crit = false;
          Particles.text(e.x, e.y - 38, '경감!', { color: '#9aa0b4', size: 11 });
        }
      } else if (dmg > e.armorCap) {
        dmg = e.armorCap;
        crit = false;
        Particles.text(e.x, e.y - 38, '경감!', { color: '#9aa0b4', size: 11 });
      }
    ...
    if (e.isBoss) {
      ...
      // v161: 온보딩 3개 층 하한을 고정 상수(2/4/5) 대신 HP에서 역산 — "상한에 걸려도 30타 남짓".
      // v165 (사장 제보 "8단계로 했는데도 왜 이렇게 쉬워? 오히려 더 안좋아졌잖아"):
      //   ★ 역산의 기준을 `e.maxHp`로 잡은 것이 치명적이었다. maxHp는 **현상금으로 부풀려진 값**이라
      //   상한도 같이 부풀어 난이도 상승을 **정확히 상쇄**했다. 실측 3층 보스:
      //     현상금 0 → HP 190 · 32타   |   현상금 8 → HP 285(+50%) · **29타 (오히려 감소)**
      //     (v160 이전에는 38타 → 57타로 정상 증가했다)
      //   즉 현상금을 최대로 올릴수록 보스가 쉬워지는 구조를 내가 만들었다.
      // → 기준을 **설계 HP(def.hp)**로 고정한다. 상한은 층 설계값에만 반응하고,
      //   현상금·막 배율로 늘어난 HP는 그대로 타수로 환산된다 (0→8단계: 32타 → 48타)
      const baseHp = (e.def && e.def.hp) || e.maxHp;
      const minCap = Dungeon.floor <= 3 ? Math.max(2, Math.round(baseHp / 30)) : 2;
    ...
    if (feel) {
      ...
      if (crit) {
        AudioSys.crit(e.x, e.y); // v174: 어디서 터졌는지가 좌우로 실린다
        // 화면 섬광은 히트스톱과 같은 리듬으로만 — 고크리 빌드에서 연속 번쩍임(눈 아픔) 방지
        if (stopped) this.critFlash = 0.07;
      } else {
        AudioSys.hit(AudioSys.mat(e.sprite), e.x, e.y); // v162: 뼈/돌/살/점액/혼 · v174: 정위·잔향·변주 3슬롯
      }
  ...
  hurtPlayer(dmg, dir, kb = 260, src = null, opt = null) {
    ...
    // ★ v213 — 사장: "층을 올라갈수록 쉬워지고 재미가 없다"
    // 실측이 이유를 짚었다. **적 HP 는 1→5층 ×4.2 로 자라는데 적 피해량은 7층까지 1로 고정**이고,
    // 그동안 사장 최대 HP 는 5 → 9 로 자란다. 한 대의 무게가 20% → 11% 로 **반토막**난다.
    //   → 오래 걸리는데(방 치우는 시간 13.6 → 25.9초) 안 죽는다.
    //     사장 F9 15런 중 사망 2회, 나머지는 전부 **포기**. 어려워서 그만둔 게 아니라 지루해서다.
    // 「어렵다」와 「지루하다」는 다르다(v206) — HP를 더 얹으면 지루함만 커진다.
    // 계단을 4층부터 시작해 **한 대의 무게를 최대 HP의 20% 근처로 유지**한다
    dmg += Dungeon.floor >= 31 ? 5 : Dungeon.floor >= 22 ? 4 : Dungeon.floor >= 14 ? 3
      : Dungeon.floor >= 8 ? 2 : Dungeon.floor >= 4 ? 1 : 0;
    ...
    // 검사 1.15(근접 리스크 보상) / 궁수 0.9 / 마도사 0.75
    // v147: 전 직업 단축 — 1.2초 무적이 피해 유입을 봉인해 죽음을 수학적으로 지웠다.
    // v155 (사장 F9 실측, 열기1 기사 1층 4연사): 그 일괄 단축이 근접 프리미엄까지 지웠다
    // (검사−궁수 격차 0.15→0.05). 1층 피해 출처 실측 = 휘두르기 58% + 접촉 32% — 근접이
    // 서 있어야 하는 자리에서 90%가 나온다. 원거리는 이 둘을 거의 안 맞는다.
    // → 검사만 1.15로 복원. 원거리 순항은 손대지 않고, 고층 위협은 피해 스케일이 계속 담당한다
    p.invuln = (p.classId === 'knight' ? 1.15 : p.classId === 'archer' ? 0.9 : 0.75) +

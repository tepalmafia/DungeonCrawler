...
const GameCombat = {
  ...
  damageEnemy(e, dmg, dir, { feel = true, crit = false, kb = 190, color } = {}) {
    ...
    if (e.isBoss) {
      ...
      // v161: 온보딩 3개 층 하한을 고정 상수(2/4/5) 대신 **HP에서 역산**한다 —
      // "상한에 걸려도 30타 남짓"이 되도록. 종전 값은 1층에서 상한 2 = 48타였고,
      // 공격 특성을 골라도 피해가 2에서 멈춰 1층 보스전이 통째로 성장 무효 구간이었다
      // (실측 1층 오스문드 95HP: 공2도 2, 공6도 2 — 성장 배수 1.00).
      // 3/6/6 → 32/29/32타. 상한 아래인 약한 빌드에는 아무 변화도 없다
      const minCap = Dungeon.floor <= 3 ? Math.max(2, Math.round(e.maxHp / 30)) : 2;

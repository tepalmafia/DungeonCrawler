...
const BOSS_DEFS = {
  ...
  2: {
    ...
    name: "시체 짐꾼 '삯꾼 몰레'", sprite: 'bossSpore', scale: 1.45, r: 32, hp: 289, speed: 34,   // v211b: 170 → 289 (아래 주석)
  ...
  3: {
    ...
    name: '간수장 바르곤', sprite: 'bossGolem', scale: 1.5, r: 33, hp: 423, speed: 30,   // v211b: 190 → 423 (아래 주석)
...
function createBoss(floor, x, y) {
  ...
  // ★ v211b — 2·3층 보스만 **너무 짧았다**. 5씨앗 평균 실측(사장 화력 수준):
  //   1층 21.3초 · 2층 **9.4초** · 3층 **7.2초** · 4층 14.3초 · 5~10층 15~19초.
  //   원인은 저작 HP 가 1~3층에서 평평했다는 것이다(190/170/190 → 화면 118/105/118).
  //   화력은 1층 공1.6 → 3층 공3.8 로 자라는데 보스만 제자리였다 —
  //   「층을 올랐는데 보스가 더 쉬워지는」 구간이었다. 2층 170→289, 3층 190→423 으로
  //   1층 118 < 2층 179 < 3층 262 < 4층 267 의 완만한 사다리를 만든다.
  //   4층 이상은 손대지 않는다 (이미 14~19초로 정상 — 재는 자를 고치니 원래 정상이었다)
  const powerAdj = 0.62;

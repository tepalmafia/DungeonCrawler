...
const World = {
  ...
  buildRoom(depth, type, floor = 1) {
    this.floor = floor;
    // 1~20층은 고유 테마, 21층+(무한 회랑)은 2막 테마를 순환
    const themeKey = floor <= 20 ? floor : ((floor - 21) % 5) + 16;
  ...
  // ── 템플릿 시스템 (M1~M4) ──
  _pickTemplate() {
    // null = 무작위 잔해 (구 방식). 층 전용(theme) 템플릿은 해당 기믹 층에서만, 가중치 ×3.
    ...
    for (let r = 0; r < 9; r++) {
      ...
      for (let c = 0; c < 18; c++) {
        ...
        if (ch === '#') {
          ...
          } else if (this.hazard === 'fog') {
            this.fogZones.push({ x: cx, y: cy, r: 34 }); // 안개 층: H = 독 안개 웅덩이
          }
        } else if (ch === 'P') {

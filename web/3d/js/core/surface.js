...
const BODY = {
  ...
  stone: /* glsl */`
    ...
    rg *= 0.75 + grit*0.5;`,
  // 사람 피부 — flesh 와 나눠 둔다. flesh 는 **썩은 살**이라 보랏빛 멍과
  // 젖은 광이 들어가는데, 그걸 주인공한테 쓰면 주인공도 시체로 보인다.
  // 산 살은 ① 잔 모공 ② 큰 얼룩이 아주 옅고 ③ **튀어나온 데가 붉다**
  // (코끝·광대·귀 — 피가 얕게 도는 자리다. 이 셋째가 제일 크다)
  skin: /* glsl */`
    float pore = fbm(lp*95.0);
    float mottle = fbm(lp*11.0);
    d *= 0.84 + mottle*0.34;
    d *= 0.93 + pore*0.16;
    d = mix(d, d*vec3(1.30,0.78,0.72), wear*0.6);
    // 수염 자국 — 턱 쪽만. 얼굴에 「면적이 다른 곳」이 하나 있어야 얼굴이다
    d *= 1.0 - smoothstep(0.55, 0.85, fbm(lp*26.0))*0.12;
    rg *= 0.72 + mottle*0.45;`,
  hair: /* glsl */`
    // 머리카락은 **한 방향으로 아주 길게 늘인 잡음**이다. 이것만으로
    // 「덩어리」가 「가닥」이 된다 — 폴리곤으로 가닥을 만들 수는 없다
    float strand = fbm(vec3(lp.x*110.0, lp.y*5.0, lp.z*110.0));
    float clump = fbm(lp*13.0);
    d *= 0.34 + strand*1.25;
    d *= 0.72 + clump*0.55;
    rg *= 0.35 + strand*0.9;`,
  wood: /* glsl */`

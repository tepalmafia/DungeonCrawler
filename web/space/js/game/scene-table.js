...
export const SCENES = {
  ...
  /**
   * ★★ **v62 에 지어졌다 — 마지막 남은 장면이었다.**
   *
   *   여덟 중 이것만 몇 판을 못 지었는데, 이유가 「공기가 계통이 아니라서」
   *   였다. v62 에 우주복이 생기면서 **공기가 규칙이 됐다** (REAL.md §2-C) —
   *   미소운석이 뚫은 방은 진공이 되고, 그 방의 문은 배가 잠그고,
   *   들어가려면 우주복을 입어야 한다.
   *
   *   ★ 갈래는 **「흩뿌려 맞았다」(두 방)** 로 연다. 한 방이면 「가서
   *     막는다」로 끝나는데, 두 방이면 **「막았는데 왜 아직 새지」**가 나고
   *     그것이 `story-table.js` 가 이 자리에 적어 둔 문장이다 —
   *     「방 하나를 못 쓴다. **고칠 곳이 문 너머에 있다**」
   */
  F: {
    key: 'F', name: '감압',
    lead: '기밀 경보',
    room: 'any', hands: '우주복을 입고 들어가 구멍을 메운다',
    uses: ['door', 'air'],
    built: true,          // game/suit.js + mission micrometeor — v62 부터 돈다
  },
  /**

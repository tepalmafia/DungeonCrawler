...
export function stepRoute(rt, dt, power = {}, opt = {}) {
  ...
  // ══ ★★★ v158 — **간 만큼 간다** (사장님 「진짜로 내가 이동하면서 싸우고
  //   있다는걸 알게 해야해」) ═══════════════════════════════════════════
  //
  //  ★★★ 여기가 `power.thrust ? 1 : LEG.coast` 였다 — **켜짐/꺼짐 두 값뿐**
  //    이라 스로틀이 한 글자도 안 들어갔다. 그래서 전속으로 밀어붙이면
  //    적은 12.4 m/s 로 다가오는데 **거점은 조금도 안 가까워졌다.**
  //
  //  ★★ v152 에서 고친 것과 **똑같은 병**이다: 속도를 아는 곳이 둘이었다.
  //    창밖과 전투는 스로틀을 읽고, 항로만 안 읽었다.
  //
  //  ★★★ 이제 **창밖과 같은 값**을 받는다 (`opt.pace` ← `speed-table.js
  //    closeK(mpsPure)`). 순항이 1 이므로 **회차 길이가 안 흔들린다** —
  //    지금까지 맞춰 둔 98분이 「순항으로 갔을 때」의 값으로 그대로 남고,
  //    밀어붙이면 짧아지고 아끼면 길어진다. 그것이 「내가 이동하고 있다」다.
  //
  //  ★ 값을 **여기 안 만든다.** 안 주면 예전 그대로 돈다 (검사와 옛 부름이
  //    안 깨진다) — `course` 를 넣을 때와 같은 규약이다
  const pace = opt.pace ?? (power.thrust ? 1 : LEG.coast);
  rt.t += opt.hold ? 0 : dt * pace * course;

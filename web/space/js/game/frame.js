...
// ══════════════════════════════════════════════════════════════════════════
//  ★★★ v153 — **배에 자리를 준다** (사장님이 세 번에 걸쳐 짚으신 것)
//
//  ★ 「**앞으로 이동 중인데 적은 그대로 뒤에 있는데?**」
//    「**그대로 서 있는 자세에서 뒤집는거면 문제가 있는거 아냐?**」
//    「**자유자재로 움직이는게 더 실감나지 않을까?**」
//
//  ══ 재 보니 **세 말씀이 뿌리가 하나**였다 ════════════════════════════════
//
//  전속으로 앞으로 가는 동안 방위별로 재 보니:
//
//      −10°  위성    192 → 173  (−19m)   가까워져야  ✔
//      118°  파편    150 → 131  (−19m)   거의 그대로 ✘
//     −132°  표류선  118 →  99  (−19m)   ★ 멀어져야  ✘
//
//  **방위와 상관없이 전부 똑같이 −19m.** 등 뒤의 것도 똑같이 다가온다.
//
//  ★★★ 까닭은 **배가 자리를 안 갖는다**는 것이다. 표적은 `(거리·방위·고도)`
//    세 숫자를 각자 들고 있고 배는 어디로도 옮겨가지 않는다 —
//    「앞으로 간다」가 실제로는 **「모두의 거리를 똑같이 깎는다」**였다.
//    v152 가 그 **깎는 속도**를 고쳤지만 **방향은 여전히 없었다.**
//
//  그래서 셋이 한 뿌리다:
//    · 뒤가 안 멀어진다        — 방향이 없으니까
//    · 회피가 서 있어도 된다    — 굴려도 **옮겨 갈 자리가 없으니까**
//    · 「자유자재로」가 안 된다  — **원리상 불가능하다**
//
//  ══ ★ 왜 여기(frame.js)에 넣나 ═════════════════════════════════════════
//
//  v98 이 이 파일을 만든 까닭이 **「자리를 아는 곳 하나」**다. 그때 자리를
//  아는 곳이 넷(표·3D·HUD·레이더)이라 네 판을 연달아 태웠고, 뼈대를 세우고
//  나서야 어긋날 수가 없는 것이 됐다. **배의 자리도 같은 곳에 둔다** —
//  `world/` 나 `main.js` 에 두면 그 순간 다섯 번째 자리가 생긴다.
// ══════════════════════════════════════════════════════════════════════════

/**
 * ★★★ **자리를 다시 각+거리로** — `pointOf` 의 **정확한 반대**.
 *
 *  ★★ 「거의 반대」면 안 된다. 두 입구가 거의 같은 것이 제일 나쁘다는 것을
 *    이 파일이 위(`relOf` 주석)에서 이미 적어 뒀다 — 정면에서만 맞는 식은
 *    **검사도 통과하고 눈에도 안 띄다가** 기수를 든 순간 어긋난다.
 *    `space-move.js [1]` 이 **왕복(pointOf → polarOf)**으로 그것을 지킨다.
 *
 * @param p { x, y, z } — 배 기준 자리 (m · −z 가 기수 앞)
 * @returns { az, el, dist } — 도 · 도 · m
 */
export function polarOf(p) {
  const x = p.x ?? 0, y = p.y ?? 0, z = p.z ?? 0;
  const dist = Math.hypot(x, y, z);
  if (dist < 1e-9) return { az: 0, el: 0, dist: 0 };
  return {
    //  ★ `pointOf` 가 `x = sin(az)·…`, `z = −cos(az)·…` 이므로 반대는
    //    `atan2(x, −z)` 다. 정면(az 0)이면 `atan2(0, +d) = 0` 으로 맞는다
    az: wrap(Math.atan2(x, -z) / DEG),
    el: Math.asin(Math.max(-1, Math.min(1, y / dist))) / DEG,
    dist,
  };
}

/**
 * ★★★ **배가 나아가면 세상이 반대로 흐른다** — 이 판의 알맹이 한 줄.
 *
 *  ★ 배를 움직이는 대신 **세상을 반대로** 옮긴다. 표적이 배 기준 좌표로
 *    적혀 있으니 그게 같은 말이고, 이러면 배의 절대 좌표를 안 들고 다녀도
 *    된다 (「우주에 원점이 어디냐」를 안 정해도 된다 — 정할 이유도 없다).
 *
 *  ★★★ **여기가 「뒤가 멀어진다」가 나오는 자리다.** 정면의 것은 `z` 가
 *    음수라 앞으로 가면 0 에 가까워지고, 등 뒤의 것은 `z` 가 양수라 **더
 *    커진다.** 옛 코드처럼 거리에서 빼기만 하면 이 부호가 통째로 없다.
 *
 *  ★★ 옆의 것은 **줄다가 늘어난다** — 스쳐 지나가는 것이 저절로 나온다.
 *    이것을 손으로 만들면 또 하나의 「자리를 아는 곳」이 되므로 안 만든다.
 *
 * @param t    { az, el, dist } — 지금 자리
 * @param step { fwd, right, up } — **이번 걸음에 배가 간 거리** (m · 기수 기준)
 * @returns 새 `{ az, el, dist }`
 */
export function flyBy(t, { fwd = 0, right = 0, up = 0 } = {}) {
  const p = pointOf(t);
  //  ★ 배가 앞으로(−z 쪽) 갔으면 세상은 +z 쪽으로 밀린다
  return polarOf({ x: p.x - right, y: p.y - up, z: p.z + fwd });
}

/**
 * ★★ **제일 가까이 스쳐 가는 자리** (CPA · closest point of approach).
 *
 *  ★ 실기 계기가 말하는 그 값이다. 「지금 몇 m」보다 **「이대로 가면 몇 m
 *    까지 붙나」**가 조종하는 사람이 실제로 쓰는 숫자다.
 *
 *  ★★ 이 값이 있어야 **「지나쳐 버렸다」를 말할 수 있다** — 지나친 뒤에는
 *    `sec` 가 음수가 된다. 옛 모형에는 이 개념 자체가 없었다 (다가오기만
 *    했으니까).
 *
 * @param t   { az, el, dist }
 * @param mps 앞으로 가는 속도 (m/s)
 * @returns `{ dist, sec }` — 제일 붙는 거리와 그때까지 남은 초.
 *          안 가고 있으면(`mps` 0) `{ dist: 지금거리, sec: Infinity }`
 */
export function closestPass(t, mps) {
  const p = pointOf(t);
  const v = Math.abs(mps);
  if (v < 1e-6) return { dist: Math.hypot(p.x, p.y, p.z), sec: Infinity };
  //  ★ 배는 −z 로 간다. 표적이 배 기준으로 +z 로 흐르므로,
  //    z(τ) = p.z + v·τ 가 0 이 되는 τ 가 제일 붙는 때다
  const sec = -p.z / v;
  return { dist: Math.hypot(p.x, p.y), sec: +sec.toFixed(4) };
}

...
// ★ v121 — 유예를 **회피에서 뽑으려면** 회피를 읽어야 한다.
//   둘 다 순수 표다 (three 를 안 쓴다) — 뼈대가 표를 읽는 것은 규약 안이다
import { DODGE } from './target-table.js';
import { RCS } from './flight-table.js';
...
export const LOCK = {
  ...
  lockFor: 2.6,
  /**
   * ══ ★★★ v121 — **유예는 「회피 두 번」을 덮어야 한다** ═══════════════
   *
   *  ★ 사장님 (2026-08-12) 「한번 타겟팅하고 일정부분 거리를 유지하면
   *    **자동 타겟팅 유지** 적용되어 있어? **회피 기동하니깐 놓치고 다시
   *    타겟팅하는 경우가 많은데?**」
   *
   *  ★★ 재 보니 **회피 한 번이 짐벌을 통째로 쓴다**:
   *      급기동 요 `AXES.yaw.rate × RCS.burst.mult` = 143°/초,
   *      회피 한 번이 `DODGE.need / mult` = 0.42초 → **60도**.
   *      짐벌이 60도이므로 **한 번이면 딱 벼랑 끝, 두 번이면 밖**이다.
   *      v119 가 「회피 한 번으로는 안 놓친다」까지만 재고 **두 번을
   *      안 쟀다** (`space-mount.js [5]` 가 한 번만 물었다).
   *
   *  ★★★ 고증이 답을 갖고 있다: 실제 레이더는 안테나가 못 보는 동안
   *    **관성으로 외삽한다**(coast). 몇 초를 그렇게 버티고, 돌아오면
   *    그대로 이어 붙인다. 우리 `grace` 가 바로 그것이고 1.1초는 **한
   *    번치**였다.
   *
   *  ★ 그래서 **회피 두 번 + 사이**를 덮게 뽑는다. 손으로 안 적는다 —
   *    나중에 회피가 빨라지면 유예가 저절로 따라온다
   */
  /**
   *  ★ 값이 **4.5** 인 까닭 — 셈이 있다:
   *      회피 두 번째가 도는 동안        0.42초
   *      + 짐벌 안으로 기수를 되돌리는 동안  0.42~0.84초
   *      + 사람이 「아, 되돌려야지」 하는 동안 ~0.5초
   *      ≈ **1.9초**  → 0.42 × 4.5 = 1.89
   *  ★★ 2.4 로 뒀다가 **1.008초**가 나와 바닥값(1.1)에 걸렸다 —
   *    즉 **아무것도 안 바뀌었다.** derive 를 해 놓고 값을 안 재면
   *    이렇게 조용히 그대로 간다
   */
  graceDodges: 4.5,
  /**
   * ★★★ v121 — **여기가 유일한 자리다.** 처음에 `combat-table` 쪽에만
   *   derive 를 걸었더니 **뼈대의 `stepLock` 은 여전히 1.1 을 읽었고**,
   *   검사가 「회피 2번에 끊김 1회」로 그대로 빨갰다. derive 를 해 놓고
   *   **쓰는 자리에 안 물리면 아무것도 안 바뀐다** — 이 저장소가 되풀이해
   *   밟는 함정이다 (표는 고쳤는데 계통이 옛 값을 읽는 것)
   */
  get grace() { return DODGE.need / RCS.burst.mult * LOCK.graceDodges; },
  /** ★ 옛 값 — 「무엇에서 얼마로 바뀌었나」를 남긴다 */
  graceWas: 1.1,
  range: 260,
...
/**
 * ★★★ v121 — **유예를 회피에서 뽑는다.** 회피 한 번이 짐벌을 다 쓰므로
 *   유예는 「회피 몇 번을 덮나」로 정해야 한다 — 초를 손으로 적으면
 *   회피 속도가 바뀔 때 **조용히 다시 놓치기 시작한다**
 */
export const graceOf = (dodgeSec) => Math.max(LOCK.grace, dodgeSec * LOCK.graceDodges);

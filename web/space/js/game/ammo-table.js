...
export const AMMO = {
  ...
  // ══ ★★★ v162 — **대폭 늘렸다** (사장님 「탄이 없어서 공격을 못하는
  //   경우가 너무 잦잖아. **기본 탄 장착 수를 대폭 늘려**」) ═════════════
  //
  //  ★★★ v133 에 열추적 6(시작 3) · 유도 4(시작 2)로 잡았다. 그때는
  //    「아끼는 것이 결심이 된다」만 봤고 **한 회차가 얼마나 긴지를 안 봤다** —
  //    2시간 12구간에 적이 구간마다 4~5번 온다. 즉 **한 회차에 50번쯤 싸우는데
  //    미사일이 다섯 발**이었다. 그건 결심이 아니라 **그냥 없는 것**이고,
  //    남는 것은 레이저 하나뿐이라 「무기가 셋」이 첫 5분에 무너졌다.
  //
  //  ★★ 그런데 **비율은 안 건드린다.** 열추적:유도가 여전히 2:1 이고
  //    파밍 무게도 3:1 이다 — 「가까운 것은 흔하고 먼 것은 귀하다」가
  //    이 표의 축이고, 그건 개수가 아니라 **사이의 비**가 만든다.
  //    늘린 것은 **분량**이지 저울이 아니다.
  //
  //  ★ 그리고 시작 비율을 0.5 → 0.8 로 올렸다. 켜자마자 반만 들고
  //    나가면 첫 구간부터 아끼게 되는데, **첫 구간은 배우는 자리**다
  ir: {
    max: 24, startAt: 0.8, farm: { w: 3, n: 5 },
    why: '가깝고 잘 놓친다 — 흔하게 주워지고 흔하게 쓴다',
  },
  arh: {
    max: 12, startAt: 0.8, farm: { w: 1, n: 2 },

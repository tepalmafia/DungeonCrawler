...
export const SPOTS = [
  ...
  // ★ **통로 자리 둘 — 「가는 길에 잠깐 붙여 둔다」가 여기서 성립한다.**
  //   한동안 통로에는 자리가 없었다. 통로 벽이 맨 벽이라 랙이 하나도
  //   없었기 때문이고, 좌표만 찍어서 허공에 자리를 만들 수는 없었다 —
  //   그러면 **번호가 안 붙은 자리**가 생기고 그건 「저기 어디」로
  //   되돌아가는 것이다. 통로에 랙을 세우고(REALSHIP §8-③) 나서 넣었다.
  //   앞뒤로 하나씩 — 조종석 쪽(3번)과 기관실 쪽(9번)
  { key: 'spine-a', room: 'spine', name: '통로 3' },
  { key: 'spine-b', room: 'spine', name: '통로 9' },

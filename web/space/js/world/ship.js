...
export function buildShip(scene, camera = null, renderer = null) {
  ...
  const marks = {
    // ══ ★★★ **항로는 v66 부터 조종석에 있다** (v82 에 안내선이 따라왔다) ══
    //  사장님 「항로 선택은 **조종석 뒤에** 두는 게 나을 것 같은데,
    //          **왜 다른 방에 있지? 멀리**」
    //  ★ 이미 조종석에 있다 — v66 에 갈래 판 둘을 콘솔 선반에 세웠다.
    //    그런데 **안내선과 가르침이 관측실을 계속 가리키고 있었다.**
    //    그래서 사장님이 그 말을 믿고 관측실까지 걸어가신 것이다.
    //    고쳐 놓고 안내를 안 고치면 **안 고친 것과 같다**
    chart: { x: 0, z: -7.6 },
    /** 관측실 해도대 — 아직 있다 (구간 전체를 넓게 보는 판). 이름만 갈랐다 */
    charttable: { x: R.observ.x0 + 1.7, z: (R.observ.z0 + R.observ.z1) / 2 },
  ...
  if (optic) {
    ...
    // ★★★ v95 — **원래 자리(창 왼쪽 위)로 되돌렸다.**
    //   사장님 「적을 확대해서 보는 화면이 **아래로 바뀌었는데 화면이
    //   안보이잔아 원래 자리로 복구해**」.

...
//
//  ★ 사장님 (2026-08-12) 「**양 옆에 검은 화면으로 우주가 가려져 있어
//    없애줘**」 (스크린샷 — 세로로 선 검은 띠 둘셋)
//
//  ══ 재서 나온 것 ══════════════════════════════════════════════════════
//
//  화면의 어두운 세로줄을 먼저 찾고 **그 화소에 광선을 쏘니** 이랬다:
//
//      (이름없음)/BoxGeometry/#0a1622/a0.35 @ 1.0m
//      (이름없음)/BoxGeometry/#0a1622/a0.35 @ 1.1m   ← **같은 판의 뒷면**
//
//  ★★ 유리를 **판이 아니라 상자**(`pane` 은 `BoxGeometry`, 두께 0.03)로
//    만들고 `DoubleSide` 를 줬다. 그래서 광선이 **앞면과 뒷면을 둘 다**
//    지나고, 35% 가 두 번 얹혀 **57.8%** 가 어두워진다. 판이 만나는
//    자리는 네 겹이라 **82%** — 그게 사장님이 보신 검은 띠다.
//
//  ★★★ **두께는 눈에 안 보이는데 어둡기는 두 배가 된다.** 유리에 두께를
//    준 것 자체가 틀린 것은 아니고(창틀에 물리려면 있어야 한다),
//    **양면을 다 그린 것**이 틀렸다. 안에서 보면 앞면 하나면 족하다.
//
//  ★ 그래서 셋을 바꾼다:
//      · `FrontSide`   — 판 하나가 **한 번만** 얹힌다
//      · `opacity 0.16` — 두 겹(모서리)이 돼도 29% 라 「유리」로 읽힌다
//      · `depthWrite: false` — 비치는 것이 뒤엣것의 깊이를 먹으면
//        정렬이 뒤집혀 **딱딱한 경계**가 생긴다 (띠의 칼 같은 모서리)
// ══════════════════════════════════════════════════════════════════════════
const GLASS = new THREE.MeshBasicMaterial({
  color: 0x0a1622, transparent: true, opacity: 0.16,

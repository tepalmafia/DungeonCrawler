...
const GROUPS = [
  ...
  ['에어록 · 광물 · 구조 신호', [
    ...
    // ★★ v62 — 우주복. **22초를 붙들고 있지 않고도** 입은 상태를 본다
    ['우주복 걸이 앞으로', (S) => { S.put(2.80, 4.85, Math.PI / 2, 0); return '걸이 앞 — 22초 잡고 있으면 입습니다'; }],
    ['우주복 입기/벗기', (S) => { const on = S.suit.on; S.putSuit(!on); return on ? '벗었습니다 — 손이 다시 빨라집니다' : '입었습니다 — 손이 0.55배로 둔합니다'; }],
    ['우주복 공기 30초', (S) => { S.putSuit(true, 30); return '공기 30초 — 곧 배가 끌어들입니다'; }],
  ]],
  ['행성 착륙', [
  ...
  ['조종 · 자동 항법', [
    ['조종간 앞으로', (S) => { S.put(0, -7.75, 0, -0.2); return '조종석 — 잡으면 수동으로 바뀝니다'; }],
  ...
  ['배 · 항로', [
    ...
    ['구간 9 (감압)', (S) => { S.setLeg(9); S.seekScene(600); return '구간 9 — 장면 A+F(적 + 감압)'; }],

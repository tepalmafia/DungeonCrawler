...
export class Input {
  constructor(canvas) {
    ...
    //   사람이 알아맞히게 만들면 안 된다.
    //
    // ★★ **그런데 그게 반대로 물었다** (2026-08-06 · 사장님 「f2 눌러서 안에
    //   항목을 눌러도 아무것도 변한게 없다고!!」). 점검 모드는 마우스를 써야
    //   해서 열 때 포인터 잠금을 푸는데, **그 안의 단추를 누르는 순간 여기가
    //   잠금을 다시 걸었다.** 커서가 사라지므로 두 번째 단추부터는 어디를
    //   눌러도 안 먹고, 그 클릭이 게임의 「집었다」로도 들어갔다.
    //
    //   그래서 규칙을 하나 더 둔다 — **`data-ui` 가 붙은 것 위에서 누른 것은
    //   게임의 클릭이 아니다.** 「어디를 눌러도 시작된다」는 그대로다:
    //   안내창·멈춤 화면은 `pointer-events: none` 이라 여기 안 걸린다.

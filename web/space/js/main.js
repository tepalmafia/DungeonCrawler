...
window.SPACE = {
   ...
   *
   *  ★ 헤드리스 브라우저가 1~2fps 라 `space-check`·`space-endtoend` 가
   *    900초를 줘도 못 끝냈다. 그 둘이 「끝까지 되나」를 묻는 검사인데
   *    **아예 안 돌고 있었다.**
   *  ★★ 1~20 으로 자른다. 20 을 넘기면 한 프레임에 1초가 넘게 흘러
   *    지나가는 것을 못 보고 뛰어넘는다 (탄이 스치는 것 같은 일).
   *  ★ **사람이 쓰는 길은 없다.** 점검 모드에도 안 넣는다 — 이건
   *    「게임을 빠르게 하는 손잡이」가 아니라 **검사를 돌게 하는 구멍**이다
   */
  fast(k = 1) { fastK = Math.max(1, Math.min(20, Number(k) || 1)); return fastK; },
...
let last = performance.now();
/**
 * ★★★ v156 — **검사용 빨리감기 배수** (1 이면 지금까지와 똑같다).
 *   `SPACE.fast(k)` 로만 바뀐다 — 사람이 하는 판에서는 영영 1 이다
 */
let fastK = 1;
function frame(now) {
  ...
  // ══ ★★★ v156 — **검사용 빨리감기** (`SPACE.fast(k)`) ═══════════════════
  //
  //  ★★★ `space-check` 와 `space-endtoend` 가 **900초를 줘도 못 끝냈다.**
  //    코드가 틀린 게 아니라, 검사가 쓰는 헤드리스 브라우저가 **1~2fps**
  //    라서 게임 시간이 실시간의 **25~60배로 느리게** 흐르기 때문이다 —
  //    한 프레임에 최대 0.05초씩만 가는데 프레임이 1초에 한두 번 온다.
  //
  //  ★★ 그래서 그 둘은 **사실상 안 도는 검사**였다. 「끝까지 되나」를
  //    아무도 안 묻고 있었던 것이고, 그건 오늘 고친 「죽은 초록」의
  //    더 나쁜 판본이다 (초록도 빨강도 아니고 **아예 안 난다**).
  //
  //  ★ **자르는 값도 같이 곱한다.** 흐른 시간만 곱하면 위의 0.05 에
  //    걸려서 아무 일도 안 난다 — 처음에 그렇게 썼다가 값이 그대로라
  //    알았다.
  //
  //  ★★★ **1 이면 지금까지와 한 글자도 다르지 않다.** 사람이 하는 판은
  //    늘 1 이고, 이 구멍은 검사만 연다. 그리고 여기 **한 곳**에만 낸다 —
  //    계통마다 「지금 빠른가」를 묻게 하면 반드시 하나를 빠뜨린다
  //    (바로 아래 멈춤·슬로우가 같은 자리에 있는 것과 같은 까닭).
  const cap = 0.05 * fastK;
  const dt0 = Math.max(0, Math.min(cap, ((now - last) / 1000) * fastK));
  last = now;

...
console.log('\n[8] 조준선이 **뭘 물었는지** 말하나');
{
  ...
  // ══ ★★ v98 — **이 줄이 각이 아닌 것을 재고 있었다** ═══════════════
  //
  //  옛 판은 `aimedAt(s, 200, 200)` 을 넣고 `off > 100` 을 봤다. 그런데
  //  고도 200도는 **있을 수 없는 기수**이고, 그때 나온 100 넘는 값은
  //  각이 아니라 **`hypot(200−115, 200−11)` 이라는 그냥 큰 수**였다.
  //  즉 검사는 통과하는데 재고 있던 것이 각이 아니었다 —
  //  이 저장소가 v90 에 이미 한 번 밟은 함정이다
  //  (「재는 것이 화면과 다르면 그 숫자로 고치면 안 된다」).
  //
  //  ★ v98 에 셈을 `frame.js` 로 모으면서 `off` 가 **진짜 구면각**이
  //    됐고(180도가 최대), 그 순간 이 줄이 울었다. 검사가 운 것이
  //    아니라 **여태 거짓으로 초록이던 것이 드러난 것**이다.
  //  ★★ 그래서 묻는 것을 바꾼다: 표적 하나를 두고 **그 반대쪽**을 보면
  //    「길에 든 것」이 아니라 **제일 가까운 것을 멀다고 말하는가**
  const one = { list: [{ id: 9, kind: 'junk', az: 20, el: 10, dist: 120, hp: 1 }] };
  const far = aimedAt(one, -160, -10);
  ok(far && far.t.id === 9 && !far.onPath && far.off > 90 && far.off <= 180,
    `등 뒤를 봐도 제일 가까운 것을 준다 — 다만 **${far.off.toFixed(0)}도 멀다고** 말한다 (길에 든 것이 아니다)`);

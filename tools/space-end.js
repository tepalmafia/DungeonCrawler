...
console.log('\n[1] ★★ **성간 공백이 정말 비어 있나**');
{
  ...
  ok(!!r, `구역이 있다 — ${r?.name}`);
  ok(r.signMult === 0, `자국이 안 쌓인다 (signMult ${r.signMult}) — 「더는 아무도 따라오지 않는다」의 실체`);
  ok(r.trackMult === 0, `붙는 속도 0 — 쫓아오던 것도 못 따라온다`);
  ok(TARGET.byRegion[VOID.region] === 0, `떠도는 것이 0 개 — 여기서는 못 번다`);
  ok(VOID.port === false, '거점이 없다 — 못 산다');
  // ★★ **v58 — 「별이 적다」로 재고 있었는데 그게 고증으로 틀렸다.**
  //   원반을 벗어나면 은하수 **띠**가 사라지지 별이 주는 것이 아니다 —
  //   먼지가 없어 오히려 또렷하다. 검사가 틀린 설계를 지키고 있었다
  //   (여섯 번째다. docs/space/REAL.md §2-H)
  ok(r.band === 0 && r.stars > 0.8 && r.fogFar > 600,
    `별 ${r.stars} · 은하수 띠 ${r.band} · 멀리까지 ${r.fogFar} —`
    + ' **별은 더 또렷한데 띠가 없다.** 하늘이 통째로 낯설어지는 것이 「나왔다」다');
  ok(SCENES.H.built === true, '장면 H 가 지어졌다');
}

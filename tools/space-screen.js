...
// ══════════════════════════════════════════════════════════════════════════
//  ★★★ v149~v150 — **잴 것이 없으면 통과가 아니라 「측정 불가」다**
//
//  ★ 사장님이 잡으셨다: 계기 넷을 통째로 없앤 척하고 돌리니
//    **「✔ 전투 화면이 비어 있다」** 가 나왔다. 이 검사의 판정이 전부
//    **「없어야 한다」 꼴**이라(원뿔이 비었나 · 잘린 것이 없나 · 겹치는 것이
//    없나), **잴 판이 없으면 넷이 다 저절로 참**이 되기 때문이다.
//
//  ★★ v149 는 이 파일 안에 손으로 넣었고, **v150 에 `tools/unmeasured.js`
//    로 옮겼다** — 같은 병이 20개 도구에 더 있어서, 「어떻게 멈추나」를
//    도구마다 적으면 어느 날 한 도구가 `exit(0)` 으로 적어 놓고 조용히
//    죽는다. `folded.js` 가 접힌 목록을 한 곳에 모은 것과 같은 까닭이다.
//    **이 파일이 하는 일은 이제 「제 목록을 넘기는 것」뿐이다.**
// ══════════════════════════════════════════════════════════════════════════
const must = measuring({
  tool: 'space-screen',
  what: '계기',
  weak: '이 검사의 판정은 전부 **「없어야 한다」** 꼴입니다\n'
    + '(원뿔이 비었나 · 잘린 것이 없나 · 겹치는 것이 없나).\n'
    + '그래서 **잴 판이 없으면 넷이 다 저절로 참**이 됩니다 —\n'
    + 'v148 까지 그 상태였고, 계기 넷을 없앤 척하고 돌리니\n'
    + '**「✔ 전투 화면이 비어 있다」** 가 나왔습니다.',
  look: '`main.js` 의 `SPACE.panels()` · `world/cockpit.js` 의 계기 셋',
});
...
try {
  ...
  for (const v of VIEWS) {
    ...
    const P = must.all(await S(() => SPACE.panels()), Object.keys(PANELS), `${v.w}×${v.h} 에서`);
  ...
  console.log('\n[★] **창이 바뀌어도 판이 같은 구석에 있나**');
  {
    ...
    for (const v of VIEWS) {
      ...
      const P = must.all(await S(() => SPACE.panels()), Object.keys(PANELS), `${v.w}×${v.h} 의 모서리를 재려는데`);
  ...
  console.log('\n[★] **딱지가 계기를 덮나** (DOM × 3D — 좌표계를 하나로 놓고)');
  {
    for (const v of VIEWS) {
      ...
      const P = must.all(await S(() => SPACE.panels()), Object.keys(PANELS), `${v.w}×${v.h} 에서 딱지를 견주려는데`);
      //  ★ 딱지가 안 떠 있으면 「아무것도 안 덮는다」가 **저절로** 참이 된다.
      //    `must` 인 딱지는 떠 있어야 재는 뜻이 있다
      for (const [nm, sp] of Object.entries(CHIPS)) {
        //  ★ 딱지는 계기가 아니다 — 이름을 바꿔 부른다 (v150)
        if (sp.must) must.as('딱지').has(C, nm, `${v.w}×${v.h} 에서`);
      }
  ...
  //  ★★★ 「못 쟀다」는 여기서 담아 **브라우저를 닫고 나서** 알린다.
  //    `process.exit()` 를 여기서 바로 부르면 `finally` 가 안 돌아
  //    브라우저가 남는다. 딴 오류는 **다시 던진다** — 여기서 삼키면
  //    진짜 고장이 「측정 불가」로 위장된다

...
const POWER_LAWS = [
  // ★ 「의도한 법칙」과 「지금 코드가 실제로 하는 것」은 다르다.
  //    FLOORS 표에 층이 셋뿐이라 floorDef() 가 4층 이상을 3층으로 클램프한다.
  //    둘을 나란히 재야 「곡선이 틀렸다」와 「표가 비었다」를 구분할 수 있다.
  ['설계 등차 1+(층−1)×0.45', 'f => 1 + (f - 1) * 0.45'],
  ['★ 지금 코드 (4층부터 클램프)', 'f => fl.floorDef(f).powerMult'],

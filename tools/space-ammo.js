...
console.log('\n[6] ★★★ **어느 거리에서 무엇이 닿나** (사장님 「공격이 안되는 이유는?」)');
{
  ...
  //  ★ v160 — 축척을 3.3배로 올리자 이 여섯이 통째로 옛 값이 됐다.
  //    **무기 사거리에서 뽑는다** (이 파일이 부르는 것은 `WEAPON_LIST` 다 —
  //    첫 판에 `WEAPONS` 를 부르려다 모듈 오류를 냈다)
  const R = Math.max(...WEAPON_LIST.map((w) => w.rMax ?? 0));
  for (const d of [0.11, 0.30, 0.41, 0.51, 0.74, 0.96].map((k) => +(R * k).toFixed(0))) {
    console.log(`   ${String(d).padStart(4)}m   ${reach(d).map((w) => `${w.slot}번 ${w.name}`).join(' · ') || '없다'}`);
  }
  ok([0.11, 0.30, 0.41, 0.51, 0.74].map((k) => R * k).every((d) => reach(d).length > 0),
    '★★★ **어느 거리에도 닿는 무기가 하나는 있다** — 없는 거리가 있으면 그 거리에서는'
    + ' 아무것도 못 하고, 그건 전투가 아니라 대기다');
  const MID = R * 0.51;
  ok(reach(MID).length === 1 && reach(MID)[0].slot === 3,
    `★★★ **${MID.toFixed(0)}m 에서는 3번 하나만 닿는다** — 사장님이 겪으신 그 자리다.`
    + ' 화면이 「너무 멉니다」라고만 하면 3번이 있다는 것을 알 길이 없다');
  //  ★ v160 — 260 을 박지 않고 **제일 긴 사거리보다 조금 밖**으로 잡는다
  const FAR = R * 1.08;
  ok(reach(FAR).length === 0,
    `★★ **${FAR.toFixed(0)}m 를 넘으면 아무것도 안 닿는다**`
    + ' — 그건 고장이 아니라 **붙어야 한다**는 뜻이고,'
    + ' 레이더가 그 거리를 보는 까닭이다');

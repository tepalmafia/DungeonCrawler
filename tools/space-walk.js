...
const CASES = [
  ...
  [0, -8.60, false, '조종석 콘솔'],
  // ★★ **(1.05, −6.95) 는 v61 에서 없앤 두 번째 좌석**이다.
  //   사장님 「운전석도 하나만 하고」 — 이 배에는 사람이 하나라 빈 좌석은
  //   없는 사람을 가리키는 물건이었다. 그 자리는 이제 **걸어다닌다.**
  //   검사가 옛 배치를 지키고 있었다 (열한 번째다) — 자리를 손으로 적지
  //   말고 **표에서 읽는다**
  // ★ v61 — 좌석이 **한가운데 하나**다 (helm-table.js HELM_SEAT.seatAt).
  //   사장님 「좌석은 센터에 있어야지」. 양옆은 이제 지나간다
  [0, -6.95, false, '조종석 좌석 (한가운데 · 하나뿐)'],
  [-1.05, -6.95, true, '옛 왼쪽 좌석 자리 — 이제 지나간다'],
  [1.05, -6.95, true, '옛 오른쪽 좌석 자리 — 이제 지나간다'],
  [0, -5.40, true, '조종석 가운데'],
  [0, 8.00, true, '통로 가운데'],

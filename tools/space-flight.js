...
console.log('\n[3] ★ **놓으면 어떻게 되나** — 축마다 다르다 (v55 + v103)');
{
  // ══ ★★★ v116 — **이 절이 v103 을 못 따라가고 있었다** ═══════════════
  //
  //  ★ 「수동이면 놓아도 **세 축이 다** 그대로다」라고 묻고 있었다.
  //    v55 에는 맞았다. 그런데 **v103 이 비행 보조를 넣었고**
  //    (사장님 「물리방향으로 움직이니 **게임이 너무 어렵게 느껴지는데?**」),
  //    그때부터 **롤만은 수동이어도 저절로 수평으로 돌아온다.**
  //  ★★ 즉 검사가 **고친 쪽을 틀렸다고 말하고** 있었다. 이 저장소가
  //    v110 에 죽은 계통을 재던 도구를 스물두 개 찾은 것과 같은 병이다 —
  //    다만 이건 더 나쁘다: **초록이 아니라 빨강으로** 거짓말을 했다.
  //  ★ 왜 롤만인가는 `horizon-table.js` 에 적혀 있다: 뒤집는 축은 롤뿐이고,
  //    롤은 겨냥에 안 쓰인다. 피치·요까지 되돌리면 그건 자동 조종이다
  const m = makeFlight();
  hold(m, 1.2, { pitch: 1, roll: 1 });
  const was = { p: m.pitch, r: m.roll };
  letGo(m, 4, { manual: true });
  ok(Math.abs(m.pitch - was.p) < 0.01,
    '★★ **피치는 수동이면 놓아도 그대로다** — 「놓으면 가운데로 돌아온다」는'
    + ' v55 에서 사장님이 고치라 하신 것이고, 그 규약은 살아 있다');
  ok(Math.abs(m.roll) < 0.05 && Math.abs(m.roll - was.r) > 0.05,
    `★★★ **롤만은 저절로 수평으로 돌아온다** (${was.r.toFixed(2)} → ${m.roll.toFixed(2)})`
    + ' — v103 의 비행 보조다 (사장님 「게임이 너무 어렵게 느껴지는데?」).'
    + ' 뒤집는 축은 롤뿐이고 롤은 겨냥에 안 쓰이므로 여기만 돕는다');
  const off = makeFlight();
  hold(off, 1.2, { roll: 1 });
  const r0 = off.roll;
  for (let t = 0; t < 4; t += DT) stepFlight(off, DT, { atSeat: false, push: {}, manual: true, assist: false });
  ok(Math.abs(off.roll - r0) < 0.01,
    '★★ 보조를 끄면 **롤도 그대로 남는다** — 보조는 규칙이 아니라 **거들기**다');
  const a = makeFlight();
...
console.log('\n[4] ★★ **짐벌** — 배가 뒤집혀도 방은 수평인가');
{
  ...
  // ══ ★★★ v116 — **2.5초로는 모자랐다** (재고 고쳤다) ═══════════════
  //
  //  ★ 재 보니 2.5초에 0.0107, 4초에 0.0003 이다. **방은 바로 서는데
  //    시간이 늘었다** — v103 의 비행 보조가 롤을 되돌리는 동안 그
  //    되돌리는 움직임이 **다시 방을 기울이기** 때문이다 (`dRoll` 이
  //    짐벌을 먹인다). 즉 「손을 놓으면 배가 알아서 수평을 잡고, 그동안
  //    사람은 그 보정을 몸으로 느낀다」 — **맞는 동작**이고 오히려 좋다.
  //  ★★ 검사가 v103 **이전 시간**을 쓰고 있었다. 고친 쪽을 틀렸다고
  //    말하는 검사는 없느니만 못하다
  letGo(f, 4, { manual: true });

...
//           **조종사 앞에 캐노피 활대가 없다** · HUD **40 × 30도** (v121)
...
function everything() {
  ...
  // ⑥ HUD — 유일하게 **정면에 있어도 되는 것.** 다만 40도를 넘으면 안 된다 (v121)
  return list;
}
...
console.log('\n[5] ★★ **HUD 가 적당한가** — 크게 만들면 가림막, 작게 만들면 안 읽힌다');
{
  // ══ ★★★ v121 — **끝을 25도에서 40도로 옮겼다** ═══════════════════
  //
  //  ★ v66 에 F-16 Block 60 값(25도)으로 못박았다. 그런데 v121 에
  //    브라우저에서 재 보니 **조종 중 카메라가 94도**라(`FLY_VIEW.fov`),
  //    25도 판은 화면 세로의 **16.5%** — 760화소 화면에서 125화소였다.
  //    거기 「15화소 글자」를 넣으면 판 높이의 0.12 를 먹고 가로로 12자다.
  //    「파편 좌 146° · 아래 22°」가 17자이므로 **넘거나 안 읽히거나**였다.
  //  ★★ 그래서 **F-35 의 가상 HUD 값 40도 × 30도**로 간다. 넓혀도
  //    고리·상자는 **화면에서 같은 크기다** — 각을 그리는 판이라
  //    판이 커진 만큼 판 대비 작아져 정확히 상쇄된다. 커지는 것은 글자뿐.
  //  ★ 그리고 v65 의 94도가 가림막이었던 것은 **넓어서가 아니라 격자를
  //    늘 깔았기** 때문이다. 그 격자는 v105 에 지웠다
  const f = hudFov();
  console.log(`   결합기 ${HUD.w} × ${HUD.h} · 눈에서 ${HUD.dist}m ⇒ **${f.h}도 × ${f.v}도**`);
  ok(f.h >= 36 && f.h <= 44, `가로 ${f.h}도 — F-35 의 가상 HUD 가 40도다 (v65 는 **94도** · v66~v120 은 25도)`);
  ok(f.v >= 26 && f.v <= 34, `세로 ${f.v}도 (v65 는 74도 · v66~v120 은 20도)`);
  ok(f.h < 50 && f.v < 40,
    '★★ 그래도 **창을 덮지는 않는다** — v65 의 94×74 가 가림막이었던 것은'
    + ' 넓어서가 아니라 격자를 늘 깔았기 때문이지만, 넓이에도 끝은 둔다');

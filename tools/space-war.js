 ...
 */
const killSec = (kind) => (KINDS[kind]?.hits ?? 6) * 0.55 * 1.6;
...
{
  //  ★ 사람 흉내: 적이 오면 사거리 안에서 부순다. 그 사이에 맞는다
  const s = fresh(31);
  setNose(s, 0);
  ...
  //   v68 은 3.7% 였다. 지금 값을 **가리지 않고 적는다** — 목표에 못 미치면
  //   못 미친다고 적어야 다음 판에서 그걸 민다
  const share = lens.reduce((a, v) => a + v, 0) / RUN;
  console.log(`   ★ 전투에 든 시간 ${(share * 100).toFixed(0)}% (v68 은 3.7% · 기획 목표 40~55%)`);
  ok(share > 0.15,
    `★★ 전투가 회차의 **${(share * 100).toFixed(0)}%** — v68 의 3.7% 에서 올라왔다.`
    + ' 목표(40~55%)까지는 아직이다: 미션 구간이 붙는 v71 에서 마저 민다');

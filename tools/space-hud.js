...
console.log('\n[5] ★★★ **화면에서 읽히나** — 판이 작다 (v105 · 사장님 「글씨가 작게 여러개」)');
{
  ...
  ok(READ.camFov >= 90,
    `★★★ **재는 화각이 조종 중 화각이다** (${READ.camFov}도) — v120 까지 72 라고`
    + ' 손으로 적혀 있었다. 조준경이 켜지는 때는 조종간을 잡았을 때이고 그때'
    + ' 카메라는 넓어져 있다. 그래서 판이 화면의 24.4% 인 줄 알았는데 **16.5%**'
    + ' 였고, 15화소인 줄 알던 글자가 12화소였다. v121 에 판을 40도로 넓혀'
    + ` 지금 **${(share * 100).toFixed(1)}%** 다`);

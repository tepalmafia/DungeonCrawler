...
console.log('\n[7] ★★★ **미터가 거짓말을 하나** (사장님 「보는것보다 m가 작게 표시된잔아」)');
{
  ...
  //  ══ ★★★ v140 — **문턱을 「목표」가 아니라 「나아졌나」로 바꿨다** ═══════
  //   v139 가 74 로 한 번에 좁혀 1.23 을 찍었는데, 사장님이 **「화면이 너무
  //   크게 변했어」**라고 하셨다 — 거리감만 고치려던 판이 **조종석 구조물까지**
  //   크게 만들었다. 그래서 86 으로 되돌렸고 지금은 1.43 이다.
  //   ★★ 문턱을 1.25 로 두면 이 절이 **빨간 채로 남는다.** 그런데 그건
  //     「못 고쳤다」가 아니라 **「덜 흔들기로 사장님이 고르셨다」**다 —
  //     빨강이 그 뜻을 못 담으면 다음에 읽는 사람이 또 20도를 움직인다.
  //   ★★★ 그래서 묻는 것을 **「94 보다 나아졌나」**로 바꾸고, **목표(1.25)와
  //     지금의 차이는 말로 적는다.** 더 갈지는 화면을 보고 정한다
  const WAS = 94 / NATURAL;
  const GOAL = 1.25;
  if (gap > GOAL) {
    console.log(`   ※ 목표는 ${GOAL} (화각 ${Math.round(NATURAL * GOAL)}도) — 지금 ${gap.toFixed(2)}.`
      + ' **한 눈금씩** 간다 (v139 에 20도를 한 번에 움직였다가 화면이 통째로 바뀌었다)');
  }
  ok(gap < WAS - 0.05,
    `★★★ **눈에는 ${gap.toFixed(2)}배 멀어 보인다** (94도 때는 ${WAS.toFixed(2)}배) —`
    + ` 100m 가 ${looksAt(100).toFixed(0)}m 처럼 보인다는 뜻이다. 사장님이`
    + ' 「**보는것보다 m가 작게 표시된잔아**」라고 하신 것이 이 값이고,'
    + ' **숫자가 아니라 렌즈가 틀린 것**이다 (`helm-table.js FLY_VIEW.fov`)');
}

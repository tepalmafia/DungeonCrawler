...
console.log('\n[3b] ★★ **행성을 박으면 끝난다** — 수동일 때만');
{
  ...
  //   ③ ★★★ v134 — 그런데 0.33 은 **문턱에서 0.02 나 떨어져 있었다.** 경보는
  //      `was <= 0.35 && near > 0.35` 인 **한 판**에만 뜨는데, 헤드리스는
  //      1초에 한두 판밖에 안 돌아 24초를 기다려도 0.02 를 다 못 기어오르는
  //      날이 있었다 — 그날 이 절이 **빨개졌고, 게임은 멀쩡했다.**
  //      낡은 빨강은 새 빨강을 덮으므로 (v120·v121 이 그랬다) **문턱 바로
  //      밑**에 놓는다. 이러면 어떤 속도로 끌려가도 **다음 한 판**에 넘는다
  await S(() => SPACE.setNear(0.3495));
...
console.log('\n[4] ★★ **행성 착륙** — 발견 · 내리기 · 싣기 · 뜨기');
{
  ...
  //
  //  ★ 여기가 **두 판을 빨간 채로** 있던 자리다. 「내린다 / 지나친다」를
  //    고르는 길이 대시의 작은 판 둘뿐이라, 좌석에서 각을 격자로 훑어도
  //    못 잡았다 (`chart0,chart1 을 못 찾았다`). 그 빨강을 두 번이나
  //    「검사가 까다로운 것」으로 흘려 들었는데, **검사가 맞았다** —
  //    사람도 그 판을 못 겨눈다.
  //  ★★★ v131 이 **갈래**에서 똑같은 병을 이미 고쳤다 (사장님 「뭘 어떻게
  //    고르라는거야? 단축키가 있어?」 → Enter). 같은 병을 같은 날 두
  //    군데서 보고 **한 군데만** 고쳤던 것이다
  const lw = await S(() => SPACE.landPickWord ?? '');
  console.log(`   화면이 하는 말 — 「${lw}」`);
  ok(/Enter/.test(lw),
    `② ★★★ **누를 키를 말한다** — 「${lw}」. 「내릴 자리가 잡혔습니다」는`
    + ' 무엇을 하라는 말이 아니다');
  await p.keyboard.press('Enter');
  ok(await until(() => SPACE.land.step === 'approach', 30, '내려가기 시작'),
    `③ ★★★ **Enter 하나로 내려간다** — 「${await said()}」. 판을 겨눌 필요가 없다`);
...
// ══════════════════════════════════════════════════════════════════════════
//  ★★★ [6r] **항로** — 길이 정말 창밖에 서나 (v130)
//
//  ★ 사장님 (2026-08-12) 「**항로가 목적지까지 희미하게 표시 되도록 해서
//    내가 목적지로 향하고 있다는 것을 시각적으로 표시**」
//    (2026-08-13) 「**중앙에 큰 원이 네비게이션이야?? 너무 시야를 가리잖아.
//    희미한 굵은 한 줄로 표시해.**」
//
//  ★★ 뼈대(`space-nav.js [7]`)는 「몇 도로 뻗고 · 얼마나 굵고 · 흐르나」를
//    잰다. **화면이 그 규칙대로 그리나**는 여기서만 나온다 — 한쪽만 읽는
//    검사는 「둘이 같나」를 못 묻는다 (v98 · `space-align.js` 가 그 자리다).
//  ★★★ 그리고 v121 이 여기서 초록이었는데도 사장님이 화면을 보고 잡으셨다.
//    그때 이 절이 잰 것은 「문이 몇 개고 얼마나 큰가」였고, **「그게 시야를
//    먹나」는 안 물었다.** 그래서 v130 에 재는 자를 바꿨다 — 크기가 아니라
//    **십자선까지의 거리**를 묻는다
// ══════════════════════════════════════════════════════════════════════════
console.log('\n[6r] ★★★ **항로** — 목적지까지 굵은 한 줄이 보이나 (v130)');
{
  await S(() => { window.__ph0 = SPACE.road().phase; });
  const r0 = await S(() => SPACE.road());
  console.log(`   표가 말하는 마디 ${r0.want} · 화면에 섰나 ${r0.shown} · 목적지 ${r0.to}`);
  console.log(`   화면에서 — 길이 ${r0.box?.len} · 십자선까지 ${r0.box?.gap}`
    + ` · 굵기 ${r0.box?.thin}~${r0.box?.wide}`);
  ok(r0.want > 1 && r0.shown === 1,
    `① ★★★ **표가 말한 길이 화면에 한 줄로 섰다** (마디 ${r0.want} → 물체 ${r0.shown}) —`
    + ' v121 은 여기가 고리 다섯이었고, 사장님이 「중앙에 큰 원이 네비게이션이야?? 너무'
    + ' 시야를 가리잖아」로 잡으셨다');
  ok(r0.box && r0.box.len > 0.15,
    `② ★★★ **한 줄로 뻗는다** (${r0.box?.len}) — 축 위에 곧게 그으면 눈과 한 줄로 서서`
    + ' 화면에서 점이 된다. 아래로 눕혀야 뻗는다 (`ROAD.drop`)');
  ok(r0.box && r0.box.gap > 0.05,
    `③ ★★★ **복판을 비운다** (십자선까지 ${r0.box?.gap}) — v121 의 제일 가까운 고리는`
    + ' 0.39 로 전투 원뿔(0.45)을 거의 다 채웠다. 「가리는 것은 진하기가 아니라 크기」다');
  ok(r0.box && r0.box.wide > 0.010 && r0.box.thin > r0.box.wide / 3,
    `④ ★★★ **굵고 고르다** (${r0.box?.thin}~${r0.box?.wide}) — 사장님 「**굵은 한 줄**」.`
    + ' 굵기를 세상 크기(m)로 못박으면 열두 배로 벌어져 줄이 아니라 **고깔**이 된다 (찍어서 알았다)');
  ok(r0.alpha > 0 && r0.alpha <= 0.09,
    `⑤ ★★★ **희미하다** (${r0.alpha}) — 이 값은 **선형**이라 눈에는`
    + ` 약 ${(r0.alpha ** (1 / 2.2)).toFixed(2)} 로 보인다. 둘을 같은 것으로 여기면`
    + ' 「희미하게 적었는데 화면은 진하다」가 난다');
  ok(r0.below.length === 2 && r0.below[0] > r0.below[1] * 5,
    `⑥ ★★ **눕혀서 원근이 산다** (축 아래로 ${r0.below[0]}° → ${r0.below[1]}°) —`
    + ' 가까운 끝은 발밑에, 먼 끝은 목적지에 붙는다');
  // ★★ 나아가면 흐르나 — 헤드리스 시계가 느리므로 **조건으로** 기다린다
...
// ══════════════════════════════════════════════════════════════════════════
//  ★★★ [6t] **처음 켠 사람의 첫 3초** — 툴팁이 저절로 뜨나 (v132)
//
//  ★ 사장님 (2026-08-13) 「**처음 시작시 툴팁 같은걸로 사용자가 이해할 수
//    있도록 도움말이 자동으로 나오도록 해야겠어.**」
//
//  ★★ 뼈대(`space-onboard.js`)는 「넉 장인가 · 적힌 키가 진짜 키인가」를
//    잰다. **정말 뜨나 · 접히나**는 여기서만 나온다
// ══════════════════════════════════════════════════════════════════════════
console.log('\n[6t] ★★★ **툴팁** — 처음 켠 사람에게 저절로 뜨나 (v132)');
{
  const o = await S(() => SPACE.onboard);
  console.log(`   떠 있나 ${o.open} · 넉 장 중 ${o.shown} 장 · 「${o.more}」`);
  //  ★ 이 검사는 **저장을 이어한 채**로 여기까지 온다 (앞 절들이 구간을
  //    밀어 놨다). 그래서 툴팁은 **안 떠 있는 것이 맞다** — 「이어하는
  //    사람에게는 안 뜬다」가 이 계통의 규약이기 때문이다
  ok(o.cards === 4, `① ★★ 넉 장이 준비돼 있다 (${o.cards}) — 다섯이면 안 읽는다`);
  ok(typeof o.more === 'string' && o.more.includes('F1'),
    `② ★★★ **「더 있다」를 알려 준다** — 「${o.more}」. 툴팁이 전부인 척하면`
    + ' 나머지를 영영 안 찾는다 (F1 이 있다는 것조차 안 알려 주고 있었다)');
  // ★★ 억지로 띄워 보고 **접히는지**까지 본다 — 나오는 길은 늘 열어 둔다
  await S(() => SPACE.showOnboard?.(true));
  await p.waitForTimeout(400);
  const on = await S(() => SPACE.onboard);
  ok(on.open, '③ ★★ 띄우면 뜬다');
  await p.keyboard.press('KeyM');
  await p.waitForTimeout(400);
  ok(!(await S(() => SPACE.onboard)).open,
    '④ ★★★ **아무 키나 누르면 접힌다** — 「들어가는 길만 있고 나오는 길이 없으면 갇힌다」');
}

// ══════════════════════════════════════════════════════════════════════════
//  ★★★ [6v] **전진은 기본 · 가속은 스태미나 · 무기마다 제 통** (v133)
//
//  ★ 사장님 (2026-08-13) 「**앞으로 이동은 기본 베이스로 하고 가속화는
//    스테미나 방식으로 … 엔진 과열이 되서 속도가 느려진다는 컨셉으로**」
//    「**레이저는 무한으로 다른 무기들도 일정 비율로 …**」
//    「**공격이 안되는 이유는?**」
//
//  ★★ 뼈대(`space-drive.js` · `space-ammo.js`)는 규칙을 잰다.
//    **게임이 그 규칙대로 도나**는 여기서만 나온다 — 특히 「과열이 계기만
//    바꾸고 배는 그대로」가 이 저장소의 단골이다
// ══════════════════════════════════════════════════════════════════════════
console.log('\n[6v] ★★★ **전진 기본 · 가속 스태미나 · 무기 통** (v133)');
{
  await S(() => { SPACE.putHelmSit(true); SPACE.setHeat(30); });
  await until(() => SPACE.helm2.k > 0.99, 30, '앉기');

  // ── ① 전진이 기본인가 ──────────────────────────────────
  const d0 = await S(() => SPACE.drive);
  console.log(`   스로틀 ${d0.throttle} (기본 ${d0.base}) · 여력 ${d0.stam} · 「${d0.word}」`);
  ok(d0.base > 0.2,
    `① ★★★ **아무것도 안 하면 ${d0.base} 로 간다** — 0 이면 매 회차 첫 동작이`
    + ' 「출발」이고, 그건 여정이 아니라 시동이다');

  // ── ② 무기마다 제 통인가 ───────────────────────────────
  const a0 = await S(() => SPACE.ammo);
  console.log(`   재고 — ${a0.word}`);
  ok(a0.ir > 0 && a0.arh > 0 && a0.ir !== undefined,
    `② ★★★ **열추적탄과 유도탄이 따로 있다** (${a0.ir} · ${a0.arh}) — v132 까지`
    + ' 한 통을 나눠 썼고, 그러면 무기가 셋이 아니라 둘이다');

  // ── ③ ★★★ 쏘면 **제 통에서만** 준다 ────────────────────
  await S(() => { SPACE.clearSky(); SPACE.putTarget('raider'); SPACE.putAim(0, 0); });
  await p.waitForTimeout(300);
  const before = await S(() => SPACE.ammo);
  await S(() => { SPACE.putWeapon(2); SPACE.fire(); });
  await p.waitForTimeout(400);
  const after = await S(() => SPACE.ammo);
  console.log(`   2번(열추적탄)을 쏘고 나서 — ${after.word}`);
  ok(after.arh === before.arh,
    '③ ★★★ **열추적탄을 쏘았는데 유도탄이 안 줄었다** — 한 통이면 여기서 같이 준다.'
    + ' 통이 갈렸다는 것을 **게임 안에서** 확인하는 자리다');

  // ── ④ ★★★ 「어느 무기가 닿나」를 말하나 (사장님 「공격이 안되는 이유는?」) ──
  //   ★ 138m 에서 레이저를 쏘시고 「너무 멉니다」만 보셨다. 맞는 말이지만
  //     **다음 손을 안 말했다** — 「안 된다」까지만 말하는 안내는 안내가 아니다
  //  ★ `putSkyDist` 는 **id** 가 필요하다. `null` 을 넘겼다가 아무 일도
  //    안 나고 조용히 지나갔다 — **조용한 무동작이 빨간 오류보다 나쁘다**
  await S(() => {
    SPACE.putWeapon(1);
    const t = SPACE.sky.list?.[0];
    if (t) SPACE.putSkyDist(t.id, 138);
  });
  await p.waitForTimeout(300);
  await S(() => SPACE.fire());
  await p.waitForTimeout(300);
  const banner = await said();
  console.log(`   138m 에서 레이저(90m)를 쏘면 — 「${banner.trim()}」`);
  ok(/멉니다/.test(banner) && /닿습니다|붙어야/.test(banner),
    '④ ★★★ **「너무 멉니다」에 다음 손이 붙는다** — 사장님이 겪으신 그 자리다.'
    + ' 사거리를 외우고 있어야만 알 수 있는 상태였다');

  // ── ⑤ ★★★ 과열되면 **배가 정말 느려지나** ──────────────
  //   ★ 계기만 바뀌고 배는 그대로인 것이 이 저장소의 단골이다.
  //     그래서 `cap`(표가 말하는 한계)과 `throttle`(정말 나가는 값)을 **둘 다** 본다
  //  ★★ **시계로 기다리지 않는다.** 헤드리스는 1초에 한두 판밖에 안 도는데
  //    스로틀을 깎는 것은 **다음 판**의 일이다 — 「0.7초 기다렸는데 1 이더라」로
  //    빨개졌고, 그건 게임이 아니라 **재는 방법**이 틀린 것이었다
  await S(() => { SPACE.putThrottle?.(1); SPACE.setHeat(100); });
  await until(() => SPACE.drive.throttle <= SPACE.drive.cap + 0.02, 40, '스로틀이 깎이는 것');
  const hot = await S(() => SPACE.drive);
  console.log(`   열 ${hot.heat01} — 최대 ${hot.cap} · 실제 ${hot.throttle} · 「${hot.word}」`);
  ok(hot.cap < 0.6,
    `⑤ ★★★ **과열이 최대 속도를 ${hot.cap} 로 깎는다** — 사장님 「과열이 되서 속도가 느려진다」`);
  ok(hot.throttle <= hot.cap + 0.02,
    `⑥ ★★★ **실제 스로틀도 따라 내려갔다** (${hot.throttle}) — 계기만 바뀌고 배가`
    + ' 그대로면 그건 고친 것이 아니라 **거짓말을 하나 더 만든 것**이다');
  ok(hot.cap > 0.2,
    `⑦ ★★ 그래도 **멈추지는 않는다** (${hot.cap}) — 0 이면 멈춰서 식기를 기다리는`
    + ' 게임이 되고, 그건 벌이 아니라 정지다');
  await S(() => { SPACE.setHeat(30); SPACE.clearSky(); });
}
...
// ══════════════════════════════════════════════════════════════════════════
//  ★★★ [6x] **어스펙트와 적 기동** — 후미가 아픈가 · 적이 꺾나 (v137)
//
//  ★ 사장님 (2026-08-13) 「실감나는 전투를 위해 무기와 **적의 기동 임펙트**,
//    **후미를 공격햇을때 데미지** 등 … 우주 전투시뮬레이션 RPG에 맞게」
//
//  ★★ 뼈대(`space-aspect.js` · `space-foe.js`)는 규칙을 잰다. **게임이 그
//    규칙대로 도나**는 여기서만 나온다 — v136 까지 `PARTS` 주석은 「뒤를
//    잡으면 엔진이 나온다」라고 v70 부터 약속하고 있었는데 **코드는 안 그랬고,
//    뼈대 검사가 없었으니 아무도 못 봤다**
// ══════════════════════════════════════════════════════════════════════════
console.log('\n[6x] ★★★ **어스펙트 · 적 기동** (v137)');
{
  await S(() => { SPACE.putHelmSit(true); SPACE.clearSky(); SPACE.putTarget('raider'); });
  await p.waitForTimeout(600);
  await S(() => { const t = SPACE.sky.list[0]; SPACE.putSkyAt(t.id, 0, 0); SPACE.putAim(0, 0); });
  await p.waitForTimeout(400);

  const t0 = await S(() => SPACE.sky.list[0]);
  console.log(`   적 하나 — 어스펙트 ${t0?.aspect}° · 기동 ${t0?.move ?? '없음'}`);
  ok(typeof t0?.aspect === 'number',
    `① ★★★ **적이 어스펙트를 갖는다** (${t0?.aspect}°) — v136 까지 「내가 적의 어느`
    + ' 쪽에 있나」를 **아무 데도 안 쟀다.** 뒤를 잡으나 마주보나 똑같았다');

  // ── ② ★★★ 뒤를 물고 있으면 **꺾는다** ─────────────────
  ok(await until(() => (SPACE.sky.list[0]?.move ?? null) !== null, 40, '적의 기동'),
    '② ★★★ **적이 기동한다** — v136 까지 적이 하는 일은 `t.az += t.vaz*dt` 두 줄,'
    + ' 즉 **표류뿐**이었다. 그건 전투가 아니라 과녁 쏘기다');
  const mv = await S(() => SPACE.sky.list[0]);
  console.log(`   기동 「${mv?.move}」 · 어스펙트 ${mv?.aspect}°`);
  ok(Math.abs((mv?.aspect ?? 180) - 180) > 0.5,
    `③ ★★ **기동이 어스펙트를 정말 바꾼다** (${mv?.aspect}°) — 안 바뀌면 그건`
    + ' 이름만 기동이고, 「뒤로 돌아 들어가는 30초」에 값이 안 생긴다');

  // ══ ★★★ ④ **후미 피해는 여기서 안 잰다** — 왜 안 재는지를 적어 둔다 ══
  //
  //  ★ 처음에 여기서 어스펙트를 세워 놓고 한 방씩 때려 맷집을 견주려 했다.
  //    그런데 헤드리스는 **1초에 한두 판**이라 탄이 나는 동안 판이 안 돌고,
  //    맞는 순간을 못 잡았다 (후미도 정면도 0.00 이 나왔다).
  //  ★★ **재는 방법이 틀린 것**이지 게임이 틀린 것이 아니다 — 그래서
  //    빨간 채로 두지 않는다. **낡은 빨강은 새 빨강을 덮는다** (v120·v121).
  //  ★★★ 피해 배수 자체는 **뼈대가 지킨다**: `space-aspect.js [2]` 가
  //    후미 ×1.45 · 측면 ×1.15 · **정면 ×0.75** 를 재고, `[5]` 가 무기 셋이
  //    서로 다르게 타는 것을 잰다. 여기서 묻는 것은 **「게임이 그 값을
  //    정말 꺼내 쓰나」**이고, 그것은 위 ①②③ (어스펙트가 있나 · 기동이
  //    도나 · 기동이 각을 바꾸나)로 이미 닿는다.
  //  ★ 되살리려면: 탄이 맞는 프레임을 게임이 알려 주는 구멍이 하나 필요하다
  //    (`SPACE.lastHit` 같은 것). 그때 이 절을 다시 연다
  await S(() => SPACE.clearSky());
}

// ══════════════════════════════════════════════════════════════════════════
//  ★★★ [6y] **가림** — 앞에 뭐가 있으면 (v142 · `docs/space/COVER.md`)
//

...
console.log('\n[7] ★★ **끝까지 간다** — 성간 공백 · 그리고 「이렇게 왔다」');
{
  ...
  //   ★ `wantStars` 는 **갈아타는 중의 값**이지 목표값이 아니다 (이름이
  //     그렇게 읽혀서 「즉시 바뀐다」고 넘겨짚었다). 구역 갈아타기는
  //     게임 시간으로 도는데 헤드리스 시계는 **1/20** 이라 한참 걸린다
  const fell = await until(() => SPACE.land.view.wantStars <= 0.2, 120, '별이 줄어들기');
  const v0 = await S(() => SPACE.land.view);
  ok(fell, `⑤ 별이 ${v0.wantStars} 까지 줄었다 — 「다 보이는데 볼 것이 없다」`);
